...
const path = 'c:/projetos/pachyx/app/contato/page.tsx';
...
export default function ContatoPage() {
  ...
  return (
    <main className="min-h-screen bg-black flex items-center justify-center p-4 overflow-hidden">
      <FadeIn delay={0.1} className="w-full max-w-3xl relative">
        <button
          onClick={() => router.back()}
...
fs.writeFileSync('c:/projetos/pachyx/scratch/update-contact.js', `
const fs = require('fs');
fs.writeFileSync('${path}', \`${newContent.replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`);
`);
